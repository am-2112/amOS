...
UINTN maxBlockAmount; /*maximum before having to allocate more memory*/
...
GPT_DISK* ValidateGPTHeader(EFI_BLOCK_IO_PROTOCOL* device) {
	...
	Status = device->ReadBlocks(device, device->Media->MediaId, 1, device->Media->BlockSize, hdr);
	if (EFI_ERROR(Status)) {
		print(L"Failed\r\n");
		free(dsk); free(hdr);
		return nullptr;
		...
		if (hdr->Signature != 0x5452415020494645) {
			...
			if (CalculatedCRC32 == CRC32) {
				...
				if (hdr->MyLBA == 1) {
					...
					if (EFI_ERROR(Status)) {
						print(L"Failed\r\n");
						free(dsk); free(hdr); free(entries);
						return nullptr;
